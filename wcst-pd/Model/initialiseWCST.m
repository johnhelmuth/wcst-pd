...
extNoise = 0.20; %0.10; % controls external (uniform noise), to PFC and to SMA
extPFC = 0.75; %0.90; %controls the external to the cog schema:
...
ctx_noise = 0.10;  % noise added to alpha_sma and beta_pfc after controls
...
reward.w_neg = 0.00;
reward.mem_feat = 0.00; % 0.00 is default (not considering past reward)                                         
epsilon.eps_str = 0.30; % learning rate for STRIATUM 0.30
epsilon.eps_sma = 0.40; % learning rate for PFC 0.40

var_dynamic = 4e2;
thr_dynamic = 4e3; 

threshold.static = 0.50;
...
satfnc.alpha_sma_hist = nan(4,lstim);
...
weights.w_STIM_PRES = 0.50;  % 0.50
             % this parameter is essential for
             % the excitation of motor schemas
             % from the environment (w_STIM_PRES)
weights.w_STIM_BASE = 0.00; %0.20  (w_STIM_BASE)

weights.w_RULE_PRES = 0.40;   % this parameter is essential
...
changeAfterCorrect = 10;  %it's the criterion
